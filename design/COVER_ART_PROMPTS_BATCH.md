# Cover art prompts — the books still needing house-style covers

> **House style = the cinematic, painted/photoreal scene** of the History Before Time covers (e.g.
> *The Calendar of Stone*): a rich, atmospheric landscape with a focal subject, golden-hour or moody
> light, depth, and a small human/figure for scale — then the gold serif title typeset on top. These
> are generated as ART (no text) in ChatGPT / Midjourney / DALL·E, then I typeset + install them.
>
> **How to hand them to me:** generate each at **2:3 portrait**, **no text baked in**, save the file,
> and tell me the path (e.g. `~/Downloads/whatever.png`). I crop to 1800×2700, typeset the title in
> the house lockup, install in every cover slot, and **commit** (covers must be committed or they
> deploy as a black placeholder).
>
> Leave a darker band at the **top** (for the series eyebrow) and the **bottom** (for the title).
> Shared palette: deep teal-to-black sky, warm African/earth gold, bronze/ochre. End MJ prompts with
> `--ar 2:3 --style raw --v 6`.

---

## Published flagships (replace the current weak/placeholder covers)

### `unheard-japan` — *The Way That Was Invented*  (currently an SVG placeholder)
> A cinematic matte-painting cover, portrait. A lone master swordsmith's forge at night in old Japan
> — the glow of the *tatara* furnace lighting a single figure in silhouette, hammer raised over a
> billet of folded steel throwing orange sparks into the dark. Beyond, a hint of a misted cedar
> mountain and a closed paper screen. Quiet, reverent, hand-made; the dignity of a craft the official
> story paints over. Deep indigo night, forge-gold light, fine grain. No text. --ar 2:3 --style raw --v 6

### `unheard-mongolia` — *The Felt and the Sky*  (currently a flat snapshot)
> A cinematic matte-painting cover, portrait. The vast Mongolian steppe at golden hour under an
> enormous sky — a single felt *ger* (yurt) tiny against the grass, a lone rider and a thread of
> horses crossing toward a far horizon, long shadows. On the skyline, almost lost, the hard
> geometry of a distant mine headframe — the fence coming for the open land. Expansive, elegiac,
> the dignity of the nomad road. Gold horizon, deep teal sky, earth-green. No text. --ar 2:3 --style raw --v 6

### `resonance` — RESONANCE (African Gold Trilogy I)  (currently an old blue-gradient cover)
> A cinematic matte-painting cover, portrait. Deep inside an ancient South African gold mine: a vast
> dark vitrified rock chamber, walls shot through with veins of glowing gold, a single shaft of warm
> light falling from far above onto one small lone figure in silhouette. Concentric rings of light
> ripple outward from where they stand, as if the stone itself rings like a struck bell. Dust motes,
> awe, first contact with something alive in the rock. Teal-black + molten gold. No text. --ar 2:3 --style raw --v 6

### `revelation` — REVELATION (African Gold Trilogy II)
> A cinematic matte-painting cover, portrait. A drowned ancient Egyptian temple half-submerged in the
> flooded Nile at dusk — colossal carved columns and a great wall of hieroglyphs rising from black
> still water, lit by a fierce forge-coloured low sun. A lone figure stands waist-deep on the central
> axis, looking up; one band of the carved symbols glows hot red-gold, as if freshly cut — the edit,
> revealed. Reflections of fire on water, embers in the air. Teal-black, ember-red, gold. No text. --ar 2:3 --style raw --v 6

### `relic` — RELIC (African Gold Trilogy III)
> A cinematic matte-painting cover, portrait. The great stone circle of Adam's Calendar on a high
> African escarpment at golden hour — towering monolithic standing stones in a ring, the central two
> framing a vast blazing golden sunburst on the horizon. At the centre, a small lone figure holds a
> small intricate golden artifact aloft; it catches the sun and blazes white-gold, sending rings of
> light through the whole circle. The stones glow molten amber; faint carvings on the ground.
> Triumphant, the key that unlocks everything — the warmest of the three. Teal sky, full molten gold,
> god-rays, lens flare. No text. --ar 2:3 --style raw --v 6

---

## Coming-soon (lighter priority — placeholders fine until then)

### `jakobus-the-long-dark` — *The Long Dark* (A Jakobus Swart story; grid-down 2030)
> A cinematic matte-painting cover, portrait. A dark South African night in a powered-down world —
> a lone tricked-out Land Cruiser parked on an empty highveld road, headlights cutting the black, a
> single weathered man (mid-60s, grey-bearded) standing beside it looking out at a dead, lightless
> town on the horizon under a brilliant, undimmed Milky Way (no light pollution left). Cold, spare,
> survival-real; the last good man at the end of the road. Deep blue-black night, one warm pool of
> headlight gold, hard stars. No text. --ar 2:3 --style raw --v 6

### `house-of-bread` — *House of Bread* (The Unheard · Holy Land)
> A cinematic matte-painting cover, portrait. The old town of Bethlehem at dusk — warm limestone
> walls, a single lit lamp, a narrow stone street climbing toward a low domed church, a faint star
> over the rooftops. In the foreground, a hand resting on ancient carved stone. Reverent, human, the
> Holy Land as a lived presence not a postcard; dignity for believers of every faith and none. Warm
> stone-gold, dusk indigo, one steady light. No text. --ar 2:3 --style raw --v 6

### `no-fear-cycle` — *Ordnance Pending* (The No-Fear Cycle · Book One)
> A cinematic matte-painting cover, portrait. Grimdark military science fiction. A vast, war-scarred
> alien battlefield at dusk — a burning city on the horizon under a bruised orange-black sky thick
> with smoke and falling ash. In the foreground a lone armoured soldier (battered powered armour,
> not a hero pose — exhausted, resolute) holds a heavy ordnance key/control rune that glows cold
> blue, the one calm light in the chaos. Distant artillery flashes; a sense of holding the line as
> the world burns. Ember-orange + ash-black + a single cold-blue glow. Epic, oppressive, heroic in
> the face of doom. No text. --ar 2:3 --style raw --v 6

### `the-salt-veil` — *The Salt Veil* (a desert epic-fantasy · Book One)
> A cinematic matte-painting cover, portrait. A desert epic-fantasy. A blinding white salt flat
> stretching to a canyon-city carved into ochre cliffs on the horizon, under a vast pale sky at low
> sun. In the foreground, three veiled women stand with their backs to us, robes moving in the wind
> — one holds a slim curved blade, the others still and watchful; power held quietly. Heat-shimmer,
> long shadows across the salt. The men hold the thrones; the women hold everything else. Bone-white
> salt, ochre stone, gold light, indigo veils. Mythic, austere, female power. No text. --ar 2:3 --style raw --v 6

---

## Companions (reverent retellings — dignified & iconic, not action covers)

### `the-song-of-the-self` — *The Song of the Self*  (a reverent retelling of the Bhagavad Gita)
> A cinematic matte-painting cover, portrait. The still moment before the great battle of the
> Bhagavad Gita: a war chariot halted between two vast armies on the dusty plain of Kurukshetra at
> dawn. The warrior Arjuna sits with his bow lowered, head bowed in doubt; beside him the calm,
> luminous figure of his charioteer Krishna, turned to counsel him — radiant but never gaudy. The
> armies are a hazy suggestion on either horizon, conches and banners catching the first light. The
> mood is reverent, interior, the hush of a soul deciding — not the clash. Warm dawn gold, dust-rose,
> deep blue shadow, a soft sacred light around the two figures. Respectful, timeless, no kitsch, no
> deity clichés. No text. --ar 2:3 --style raw --v 6

### `the-wrath-of-achilles` — *The Wrath of Achilles*  (the Iliad, plainly told)
> A cinematic matte-painting cover, portrait. The Iliad in one image: a lone Greek warrior in bronze
> armour stands on the windswept shore below the walls of Troy at dusk, his back to us, looking up at
> the distant burning city across the plain. Black ships are drawn up on the dark beach behind him;
> his shield rests at his side, his grief and rage held in his stillness. Far off, the high towers of
> Ilium glow against a bruised sky. Elegiac, monumental, the weight of fate and a single man's anger —
> the cost of glory. Bronze and ember, wine-dark sea, ash-grey sky, one shaft of failing gold light.
> Homeric, austere, no fantasy armour. No text. --ar 2:3 --style raw --v 6

### `the-walls-of-uruk` — *The Walls of Uruk*  (the Epic of Gilgamesh, plainly told)
> A cinematic matte-painting cover, portrait. The Epic of Gilgamesh in one image: the baked-brick
> walls of Uruk rise at dusk above the southern Mesopotamian plain; a lone king stands on the
> rampart with his back to us, empty-handed after the failed search for immortality, looking out
> over the city he built. Far below, the plain goes bronze and violet; a thin ribbon of river
> catches the last light. No fantasy armour, no cartoon gods — austere, elegiac, the weight of
> friendship lost and brick that remains. Clay-red, dusk gold, deep indigo shadow. Monumental and
> quiet. No text. --ar 2:3 --style raw --v 6

---

## The Reichenbach Files (present-day Sherlock — modern London noir, NOT Victorian)

> Shared look for the series: contemporary London, cinematic crime-thriller, moody and rain-slick;
> a recurring cold-blue / amber palette and a recurring lone-figure-in-the-city motif so the five
> read as a set. Realistic and grounded — no deerstalker, no gaslamp pastiche, no fantasy. Leave a
> darker band top and bottom for the eyebrow + title. End each with `--ar 2:3 --style raw --v 6`.

### `modern-sherlock` — *The Scarlet Thread*  (Book One · after *A Study in Scarlet*)
> A cinematic crime-thriller cover, portrait. Present-day London at night in the rain. A lone tall
> figure in a long dark coat stands at the mouth of a wet, neon-lit alley, seen from behind, phone
> light in hand; on the dripping brick wall ahead, a single word is scrawled large in blood-red —
> the only colour in a desaturated blue-black scene. Reflections on the slick tarmac, distant city
> glow, a sense of a mind reading the whole street at once. Noir, electric, modern Holmes. Deep
> blue-black, one bleed of scarlet. No text. --ar 2:3 --style raw --v 6

### `modern-sherlock-2` — *The Poisoned Fortune*  (Book Two · after *The Sign of the Four*)
> A cinematic crime-thriller cover, portrait. Present-day London, dusk. A grand inherited townhouse
> or a glass private-bank office glows cold from within; in the foreground, on a polished table, a
> single crystal tumbler of amber spirits catches the light — beautiful and wrong, a faint oily
> sheen on its surface. A lone silhouette is reflected in the dark window beyond. Wealth, betrayal,
> a death dressed as a gift. Cold blues and one warm poisoned amber, deep shadow. No text. --ar 2:3 --style raw --v 6

### `modern-sherlock-3` — *The Viral Haunting*  (Book Three · after *The Hound of the Baskervilles*)
> A cinematic crime-thriller cover, portrait. A lonely modern moor at night under a sick green-grey
> fog; a single figure stands small on a wet path, phone-torch beam cutting the murk, looking toward
> a low shape half-seen in the mist — could be a huge hound, could be nothing, deliberately ambiguous.
> Pixels/static bleed faintly at the edges of the fog as if the dread is spreading like a signal. The
> rational mind against a contagion of fear. Desaturated, eerie green-grey, one cold torch-white beam.
> No text. --ar 2:3 --style raw --v 6

### `modern-sherlock-4` — *The Woman Who Beat Him*  (Book Four · after *A Scandal in Bohemia*)
> A cinematic crime-thriller cover, portrait. Present-day London, night. A poised woman in a sharp
> dark coat walks away from camera into the lit city, unhurried, utterly in control, glancing back
> over one shoulder with a faint knowing half-smile — she is the one who got away clean. Behind her,
> blurred, a tall male figure stands still, outplayed. Elegant, electric, the equal-and-opposite.
> Cold city blue, one stroke of warm gold from a streetlight on her face. No text. --ar 2:3 --style raw --v 6

### `modern-sherlock-5` — *The Reichenbach Protocol*  (Book Five · after *The Final Problem* / *The Empty House*)
> A cinematic crime-thriller cover, portrait. The rain-lashed rooftop of a tall modern London tower
> at night, the city a vast glittering grid far below. Two figures face each other at the very edge
> in the downpour — one tall in a long coat, one the adversary — locked, the end of everything between
> them. Lightning-lit clouds, vertiginous drop, a waterfall of rain off the parapet echoing the
> Reichenbach fall. The reckoning the whole series is named for. Storm-blue and black, one cold flash
> of white. No text. --ar 2:3 --style raw --v 6
