...
- **Never commit heavy binaries.** Images, PDFs, EPUBs, audio, video, fonts and archives
  live in Cloudflare R2 (`arjuna-badger-prod`), tracked by `assets.manifest.json`.
  A pre-commit gate (`scripts/asset_gate.py`) blocks them — do not work around it.
  *(This reverses the old "commit EPUB/PDF exports" rule, which is what grew this repo
  to 10 GB. `build/BOOK.md` stays committed: it is text.)*
- **Never commit build output** — `books/*/build/**` binaries are regenerated by
  `books/*/build.py`; archived offline, never versioned, at any size.
- Adding an asset: `./tools/asset_manifest.py build press` → `./tools/r2_push.py push press`
  → `git add assets.manifest.json`. Commit the contract, not the bytes.
- Never commit `site/public/` — CI rebuilds it
- Stage specifically; review before `git add -A`
- New clone? Run `./scripts/install-hooks.sh` once — git does not clone hooks.
