#!/usr/bin/env python3
"""Merge *The Walls of Uruk* book/*.md -> build/BOOK.md.
...
def merge() -> Path:
    ...
    if len(sections) < 2:
        sys.exit("  [skip] no sections in book/")

    title_block = f"""# {TITLE}

*The Epic of Gilgamesh, plainly told — a companion for every reader*
...
"""
    ...
    for p in sections:
        ...
        # Chapters reference plates relative to book/ (../design/plates/…). The render gate
        # runs pandoc from a temp copy, so BOOK.md carries absolute paths (house convention;
        # the site build maps them back to repo files for the read-online page).
        text = text.replace("](../design/", f"]({HERE / 'design'}/")
        parts.append(text)
