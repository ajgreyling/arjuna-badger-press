#!/usr/bin/env python3
"""Lay the HBT title typography onto the cinematic cover plate for *The First Fire*.

Reads design/cover-plate.png (a text-free dawn-fire matte painting, generator TBD — a lit havan
flame at first light, in the same cinematic register as the Song of the Self's Kurukshetra plate),
adds soft top/bottom legibility scrims and the house serif typography (Didot title / Cochin
eyebrow + author), and writes the typeset cover to design/cover.{png,jpg} + build/export/cover.{png,jpg}.
Same house method as The Song of the Self / The Jakobus File / The Silver Thread. Re-runnable:
always works from the plate. Requires design/cover-plate.png to exist first (not yet generated).
...
def main() -> None:
    ...
    sub = "A reverent companion to the Four Vedas"
    sub_y = ty + 2 * lh + 18
