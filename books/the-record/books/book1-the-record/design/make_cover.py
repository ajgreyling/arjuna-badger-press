#!/usr/bin/env python3
"""Compose the final cover for ONE RECORD, Book I: *The Record*."""
...
def main() -> None:
    ...
    # Deepen only the type bands; retain the threshold, token and watch as the image's evidence.
    scrim = Image.new("RGBA", image.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(scrim)
    top_end = int(H * 0.28)
    for y in range(top_end):
        alpha = int(168 * (1 - y / top_end) ** 1.12)
        draw.line(((0, y), (W, y)), fill=(2, 7, 11, alpha))
    bottom_start = int(H * 0.86)
    for y in range(bottom_start, H):
        alpha = int(176 * ((y - bottom_start) / (H - bottom_start)) ** 1.15)
        draw.line(((0, y), (W, y)), fill=(2, 7, 11, alpha))
    image = Image.alpha_composite(image, scrim)

    eyebrow = font(ATK_REG, 42)
    image = draw_tracked(image, cx, 122, "ONE RECORD · BOOK I", eyebrow, 10, INK)
    rule = ImageDraw.Draw(image)
    rule.line(((cx - 175, 190), (cx + 175, 190)), fill=INK, width=2)

    title = font(ATK_BOLD, 174)
    image = draw_tracked(image, cx, 224, "THE RECORD", title, 9, INK, glow=8)

    author = font(ATK_REG, 55)
    image = draw_tracked(image, cx, 2482, "ANDRIES J. GREYLING", author, 9, INK)

    # The publisher's Badger Bow mark is part of the deterministic composition.
