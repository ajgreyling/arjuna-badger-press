#!/usr/bin/env python3
"""Lay title + author typography onto the clean cover plate for *Kookie*.

    python3 books/kookie/design/make_cover.py
"""
...
def main() -> None:
    ...
    # Pale desert sky needs a firm top band; foot seats author over sand.
    scrim = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    sd = ImageDraw.Draw(scrim)
    top_end = int(H * 0.48)
    for y in range(top_end):
        a = int(200 * (1 - y / top_end) ** 1.3)
        sd.line([(0, y), (W, y)], fill=(28, 22, 14, a))
    bot_start = int(H * 0.80)
    for y in range(bot_start, H):
        a = int(200 * ((y - bot_start) / (H - bot_start)) ** 1.2)
