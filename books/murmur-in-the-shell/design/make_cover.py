#!/usr/bin/env python3
"""Lay the title + author typography onto the clean cover plate for *Murmur in the Shell*.

Reads design/cover-plate.png (the data-hall / nerve-signal plate — text-free), fits + center-
crops to house size, adds legibility scrims and house serif type, and writes the typeset cover
to design/cover.{png,jpg} + build/export/cover.{png,jpg}. Same house method as Henry Sugar.

Eyebrow = "A SYSTEMS THRILLER"; Bone-white title ink; cool machine-teal accent against the
data-hall plate.
...
TEAL = (140, 200, 210, 255)       # cool machine-glow accent
SHADOW = (4, 8, 10, 210)
...
def main() -> None:
    ...
    for y in range(top_end):
        a = int(180 * (1 - y / top_end) ** 1.4)
        sd.line([(0, y), (W_, y)], fill=(4, 8, 10, a))
    bot_start = int(H_ * 0.83)
    for y in range(bot_start, H_):
        a = int(175 * ((y - bot_start) / (H_ - bot_start)) ** 1.3)
        sd.line([(0, y), (W_, y)], fill=(3, 6, 8, a))
    img = Image.alpha_composite(img, scrim)

    draw = ImageDraw.Draw(img)

    f_eyebrow = font(COPPER, 30)
    draw_tracked(draw, cx, int(H_ * 0.058), "A SYSTEMS THRILLER", f_eyebrow, 7, TEAL)

    rule_y = int(H_ * 0.058) + 48
    rw = 150
    draw.line([(cx - rw, rule_y), (cx + rw, rule_y)], fill=TEAL, width=2)

    f_title = font(DIDOT, 118)
    lines = ["MURMUR", "IN THE", "SHELL"]
    ty = int(H_ * 0.115)
    lh = 130
    ...
    draw_tracked(draw, cx, ty + 3 * lh + 14, "the one form of resistance a machine cannot route around",
                 f_tag, 1, TEAL)
