#!/usr/bin/env python3
"""Cover for *The Oyster in the Machine* — house colophon treatment (no commissioned art).

    python3 books/oyster-in-the-machine/design/make_cover.py
"""
...
def main() -> None:
    ...
    canvas.paste(mark, ((W - mw) // 2, 190), mark)

    y = 1350
    f_title = font("AtkinsonHyperlegible-Bold.otf", 108)
    for line in TITLE.split("\n"):
        y = centre(d, y, line, f_title, IVORY) + 40

    y += 34
    d.line([(W // 2 - 150, y), (W // 2 + 150, y)], fill=GOLD_DIM, width=3)
    y += 58

    y = centre(d, y, SUB, font("AtkinsonHyperlegible-Italic.otf", 48), GOLD) + 10
