#!/usr/bin/env python3
"""Lay title + author typography onto the clean cover plate for *The Long Road*.

    python3 books/the-long-road/design/make_cover.py
"""
...
def main() -> None:
    ...
    # Pale sky + bright road: deepen top for title, foot for author.
    scrim = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    sd = ImageDraw.Draw(scrim)
    top_end = int(H * 0.50)
    for y in range(top_end):
        a = int(205 * (1 - y / top_end) ** 1.35)
        sd.line([(0, y), (W, y)], fill=(22, 28, 36, a))
    bot_start = int(H * 0.80)
    for y in range(bot_start, H):
        a = int(205 * ((y - bot_start) / (H - bot_start)) ** 1.2)
