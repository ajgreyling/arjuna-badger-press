#!/usr/bin/env python3
"""Lay the title + author typography onto the clean cover plate for *The Dreaming*.

Reads design/cover-plate.png (the dreaming-mind plate — text-free), adds soft legibility scrims
and house serif typography, and writes the typeset cover to design/cover.{png,jpg} +
build/export/cover.{png,jpg}. Same house method as *A Man They All Read Wrong* / *Henry Sugar*.

Faithful Modern shelf (no series number): eyebrow = "FAITHFUL MODERN"; indigo/bone ink to sit
with the plate's midnight palette.
...
INK = (236, 232, 244, 255)        # cool bone-white title ink
ACCENT = (150, 160, 214, 255)     # soft indigo for eyebrow + tagline
SHADOW = (6, 6, 14, 215)
...
def main() -> None:
    ...
    for y in range(top_end):
        ...
        sd.line([(0, y), (W, y)], fill=(8, 8, 18, a))
    bot_start = int(H * 0.85)
    for y in range(bot_start, H):
        a = int(150 * ((y - bot_start) / (H - bot_start)) ** 1.3)
        sd.line([(0, y), (W, y)], fill=(6, 6, 14, a))
    img = Image.alpha_composite(img, scrim)

    draw = ImageDraw.Draw(img)

    # Eyebrow — the imprint, no series number.
    f_eyebrow = font(COPPER, 30)
    draw_tracked(draw, cx, int(H * 0.055), "FAITHFUL MODERN", f_eyebrow, 8, ACCENT)
    ...
    # Tagline — the book's line, in indigo italic.
    f_tag = font(ATK_ITAL, 38)
    draw_tracked(draw, cx, ty + 2 * lh + 16, "to keep the lesson, you let go of the lecture", f_tag, 1, ACCENT)
