#!/usr/bin/env python3
"""Lay title + author typography onto the clean cover plate for *Palindrome*.

    python3 books/palindrome/design/make_cover.py
"""
...
def main() -> None:
    ...
    # Room is already dark; light top deepen + foot band for author.
    scrim = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    sd = ImageDraw.Draw(scrim)
    top_end = int(H * 0.28)
    for y in range(top_end):
        a = int(160 * (1 - y / top_end) ** 1.2)
        sd.line([(0, y), (W, y)], fill=(6, 5, 4, a))
    bot_start = int(H * 0.82)
    for y in range(bot_start, H):
        a = int(190 * ((y - bot_start) / (H - bot_start)) ** 1.15)
        sd.line([(0, y), (W, y)], fill=(6, 5, 4, a))
    img = Image.alpha_composite(img, scrim)

    f_eyebrow = font(ATK_REG, 40)
    img = draw_tracked(img, cx, int(H * 0.052), "A CHAMBER PIECE", f_eyebrow, 10, INK)

    rule_y = int(H * 0.052) + 58
    rd = ImageDraw.Draw(img)
    rd.line([(cx - 140, rule_y), (cx + 140, rule_y)], fill=INK, width=2)

    ty = int(H * 0.095)
    f_title = font(ATK_BOLD, 118)
    img = draw_tracked(img, cx, ty, "PALINDROME", f_title, 8, INK)

    sub_y = ty + 155
    f_sub = font(ATK_ITAL, 42)
    img = draw_tracked(img, cx, sub_y, "one room · one night · four men", f_sub, 1, INK)
