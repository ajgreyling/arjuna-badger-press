#!/usr/bin/env python3
"""Render and master the Afrikaans Palindrome novella with Emma Lilliana (ElevenLabs).

Source: books/palindrome/build/BOOK.af.md
Voice / model / settings match Die Vuur in die Donker / Die Laaste Strooi.

Usage:
    python3 books/palindrome/audio/render_emma_afrikaans_masters.py --dry-run
    python3 books/palindrome/audio/render_emma_afrikaans_masters.py
"""
...
BOOK_PATH = AUDIO_DIR.parent / "build" / "BOOK.af.md"
FRONTMATTER_PATH = AUDIO_DIR / "afrikaans-kopiereg-voorblad.md"  # optional
BACKMATTER_PATH = AUDIO_DIR / "afrikaans-faktuele-nawoord.md"  # optional
...
def build_manifest(sections: list[Section], dry_run: bool = False) -> dict[str, Any]:
    return {
        ...
        "full_mastered_file": str(OUT_DIR / "palindrome-afrikaans-emma-master.mp3"),
        "failures": [],
    }
...
def enrich_manifest_with_results(
    ...
    failures = []
    for section, chapter in zip(sections, manifest["chapters"]):
