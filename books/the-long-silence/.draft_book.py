#!/usr/bin/env python3
"""Metered single-shot draft driver for *The Long Silence*.
...
def draft_one(ch: dict, story_state: str, canon: str, template: str) -> str:
    ...
    prompt = (
        ...
        + template.format(
            ...
            MOVEMENT=ch.get("movement", ""),
            POV=ch.get("pov", ""),
            SETTING=ch.get("setting", ""),
            BEATS="; ".join(beats),
            TURN=ch.get("turn", ""),
            DETONATION=ch.get("detonation", "no"),
    ...
    system = (
        "You are the Scribe for The Long Silence — Liu Cixin Dark Forest engine (theory only), original "
        "prose, native English, no sag, idea-as-detonation. Markdown chapter output only. Never state the "
        "theme; the aliens stay withheld."
    )
...
def compress_state(prev: str, ch_id: str, title: str, text: str) -> str:
    prompt = f"""Update rolling STORY-STATE for *The Long Silence* after {ch_id} ("{title}").
Keep <= 600 words: plot position (which movement/beat), the relay (which runner carries it; who has died),
mandates check (#1 native English, #2 no sag), threads planted/paid (PP-IDs), what the reader knows vs the
cast (negative-space law), Chekhov guns still open, runner states. Bullets only.
