# Source & Reality Notes — *YESTERDAY, TOMORROW*

> Appendix B of the bible. Development references; **not an endorsement by the referenced
> institutions.**

Primary public references consulted for this scaffold:

- `the420code.org` — public axiom, physics claims, notebooks, proofs and kill-switch register
- `the420code.org/prereg/` — frozen prediction register and explicit live/fired conditions
- `physics.nist.gov/constants` — official CODATA recommended values
- `lucid.rodeo/the420code/lucid/` — current Lucid description and operating claims
- `lucid.rodeo/the420code/ledger/` — Coherence Ledger demonstration and honest limits
- `lucid.rodeo/the420code/about/` — current pillars, prototypes, concepts and non-claims
- `lucid.rodeo/the420code/proofs/ethical-casino/` — paper-trading teaching instrument and explicit no-edge boundary
- `technopark.org.za` — Technopark's real business-park context; company and location details require final fact-check before publication

> **LEGAL/EDITORIAL NOTE** — All university, scientific-infrastructure, company and institutional
> participation in the novels is **fictional unless separately licensed or confirmed**. Use fictional
> consortium entities where real-world attribution could imply endorsement or misconduct.
